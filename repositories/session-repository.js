...
class SessionRepository {
    ...
    async readSessions(query) {
        let sessions;
        try {
            sessions = await this.sessionDao.find(query).populate('participants', '_id firstname lastname emailAddress').populate('currentUser', '_id firstname lastname').populate('theme', 'organisers');
        } catch (err) {
            throw new Error('Unexpected error occurred. ' + err);
        }
